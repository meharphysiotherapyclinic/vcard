...
function getAIResponse(msg) {
  ...
  if (/^(hi|hello|hey|hii|physio|doctor|consultation|help)$/.test(msg)) {
    return "Hello! 👋 Welcome to Mehar Physiotherapy Clinic. How can we help you today? You can inquire about our <strong>Clinic Timings</strong>, <strong>Home Visits</strong>, or <strong>Book an Appointment</strong> directly below:" + wrap(waLink);
  }
  ...
  if (msg.includes("location") || msg.includes("where") || msg.includes("address") || msg.includes("map") || msg.includes("reach") || msg.includes("direction") || msg.includes("gaur city")) {
    return "We are located at <strong>F-42, 16th Avenue, Gaur City-2, Greater Noida West</strong>. You can find our exact location on the map here:" + wrap(mapLink);
  }
  ...
  if (msg.includes("appointment") || msg.includes("book") || msg.includes("consult") || msg.includes("contact") || msg.includes("referral") || msg.includes("doctor") || msg.includes("visit")) {
    return "You can simply call or WhatsApp us to confirm your appointment:" + wrap(callLink, waLink);
  }
  ...
  if (msg.includes("fee") || msg.includes("charge") || msg.includes("cost") || msg.includes("price") || msg.includes("how many") || msg.includes("sessions") || msg.includes("long") || msg.includes("duration")) {
    if (msg.includes("how many") || msg.includes("sessions") || msg.includes("long")) {
        return "A typical session lasts <strong>30-45 minutes</strong>. The total number of sessions depends on your recovery progress. Dr. Arneja will guide you during the first assessment." + wrap(callLink);
    }
    return "Treatment charges are very competitive. For a specific quote for Clinic or Home visits, please contact us:" + wrap(callLink);
  }
  ...
  if (
    ...
    return "Dr. Arneja specializes in treating your condition and advanced rehabilitation. We use safe, modern techniques to ensure fast recovery. <br><br><strong>Book your assessment now:</strong>" + wrap(callLink, waLink);
  }

  // 8. PAIN & SAFETY 
  if (msg.includes("hurt") || msg.includes("scared") || msg.includes("safe") || msg.includes("clean") || msg.includes("hygiene")) {
    return "Your safety is our priority. We follow strict <strong>sanitization protocols</strong>, and our physiotherapy techniques are designed to <strong>relieve pain</strong>, not cause it." + wrap(callLink);
  }
  ...
  if (msg.includes("senior") || msg.includes("old") || msg.includes("elderly") || msg.includes("parent") || msg.includes("balance") || msg.includes("fall")) {
    return "We offer dedicated care for <strong>Senior Citizens</strong> focusing on balance, mobility, and fall prevention. We recommend a home visit for maximum comfort." + wrap(callLink, waLink);
  }

  // 10. THANK YOU
  if (msg.includes("thank") || msg.includes("thx") || msg.includes("thanks")) {
    return "You're very welcome! 😊 We look forward to helping you stay fit and pain-free." + wrap(callLink);
  }

  // DEFAULT FALLBACK
  return "I want to make sure you get the right help! For any medical questions or to book a visit, please contact Dr. Arneja directly:" + wrap(callLink, waLink);
}
